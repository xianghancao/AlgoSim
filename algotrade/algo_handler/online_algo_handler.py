...
class OnlineAlgoHandler(AlgoHandler):
    ...
    def _excute_algo(self, event):
        ...
        factor = (pd.to_datetime("2020-06-12 14:55:00") - pd.to_datetime(self.timestamp)).seconds
        if factor<600 and factor>0:
            factor = factor/600
        else:
            factor = 1
        ...
        for i in range(len(self.ticker_names)):
            ...
            # 建仓
            if score < -0.001 and -1 * score - spread[i] - fees + 0.0005*factor > 0 and spread[i]>0: # and self.position[ticker]['净开仓']>=0:
            ...
            if score > 0.001 and score - spread[i] - fees +0.0005*factor > 0 and spread[i]>0:
            ...
            # 平仓
            if score > 0 and score - spread[i]/2 - 0.0005*factor  > 0 \
            ...
            if score < 0 and -1 * score - spread[i]/2 - 0.0005*factor  > 0 \
                and self.position[ticker]['净开仓'] > 0 \
                and spread[i]>0 \
